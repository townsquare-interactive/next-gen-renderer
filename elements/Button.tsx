...
export const Button = (props: SingleButtonProps) => {
    ...
    console.log('btn', btn.opensModal, pageModalVars)

    const isModalBtn = pageModalVars && btn.opensModal != -1 ? true : false
    return (
        <>
            {/*  {(pageModalVars && btn.opensModal) === 0 && <button onClick={pageModalVars[0].close}>Modal time</button>} */}
            <Link
                href={btn.link || ''}
                ...
                onClick={isModalBtn && pageModalVars && btn.opensModal != -1 ? pageModalVars[btn.opensModal || -1].close : () => {}}
