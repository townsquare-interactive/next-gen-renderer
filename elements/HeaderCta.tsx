...
const HeaderCTA = (props: HeaderCTAProps) => {
    ...
    const sharedClasses = ['cta', cta.type === 'banner' ? 'cta-banner' : btnCount ? `cta-${btnCount}` : '']
        ...
        >
            <ConditionalWrapper
                ...
                trueOutput={(children: ReactChild) => (
                    <Link href={cta.link || ''} className={cn(sharedClasses)} style={{ backgroundColor: cta.bgColor }}>
                        {children}
                    </Link>
                )}
                falseOutput={(children: ReactChild) => (
                    <span className={cn(sharedClasses)} style={{ backgroundColor: cta.bgColor }}>
                        {children}
                    </span>
                )}
            >
